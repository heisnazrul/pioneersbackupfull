...
<div class="main-content py-10">
  ...
  <form action="{{ route('admin.accommodations.update', $accommodation) }}" method="POST" class="space-y-6">
    @csrf
    @method('PUT')
      ...
      <div>
        ...
        <select name="school_branch_id" class="mt-1 block w-full border rounded-md px-3 py-2" required>
          @foreach($branches as $branch)
            <option value="{{ $branch->id }}" @selected(old('school_branch_id', $accommodation->school_branch_id) == $branch->id)>
