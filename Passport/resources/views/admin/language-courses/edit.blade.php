...
<div class="main-content py-10">
  <div class="flex justify-between py-10">
    ...
    <a href="{{ route('admin.language-courses.index') }}" class="ti-btn rounded-full ti-btn-outline ti-btn-outline-primary">Back to List</a>
  </div>
  ...
  <form action="{{ route('admin.language-courses.update', $course) }}" method="POST" class="space-y-4">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input type="text" name="slug" value="{{ old('slug', $course->slug) }}" class="mt-1 block w-full border rounded-md px-3 py-2" required>
      </div>
      <div>
    ...
    <div class="flex space-x-2">
      ...
      <a href="{{ route('admin.language-courses.index') }}" class="ti-btn rounded-full border">Cancel</a>
    </div>
  </form>
</div>
@endsection
