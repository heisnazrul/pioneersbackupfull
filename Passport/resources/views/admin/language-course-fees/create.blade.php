...
<div class="main-content py-10">
  <div class="flex justify-between py-10">
    ...
    <a href="{{ route('admin.language-course-fees.index') }}" class="ti-btn rounded-full ti-btn-outline ti-btn-outline-primary">Back to List</a>
  </div>
  ...
  <form action="{{ route('admin.language-course-fees.store') }}" method="POST" class="space-y-4">
    @csrf
      ...
      <select name="language_course_id" id="language_course_id" class="mt-1 block w-full border rounded-md px-3 py-2" required>
        ...
        @foreach($courses as $course)
          <option value="{{ $course->id }}" @selected(old('language_course_id', $selectedCourseId ?? null) == $course->id)>
            {{ $course->name }} — {{ optional($course->branch->school)->name }}
          </option>
        @endforeach
      </select>
    </div>
    ...
    <div class="flex space-x-2">
      ...
      <a href="{{ route('admin.language-course-fees.index') }}" class="ti-btn rounded-full border">Cancel</a>
    </div>
  </form>
</div>
...
<script>
  ...
  async function loadCourseFees(courseId) {
    ...
    try {
      const url = `{{ route('admin.language-course-fees.fetch', ['languageCourse' => 'COURSE_ID']) }}`.replace('COURSE_ID', courseId);
