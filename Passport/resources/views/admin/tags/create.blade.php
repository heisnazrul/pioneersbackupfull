    ...
    <form action="{{ route('admin.tags.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="form-group">
                ...
                <div class="flex items-center gap-3 mt-1">
                    <input type="color" name="color" id="color" value="{{ old('color', '#6366f1') }}" class="h-10 w-16 border rounded">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="form-group">
                ...
                <textarea name="description" id="description" rows="3" class="mt-1 block w-full border rounded-md px-3 py-2">{{ old('description') }}</textarea>
            </div>
            <div class="form-group">
                <label for="ar_description" class="block text-sm font-medium text-gray-700">Arabic Description</label>
                <textarea name="ar_description" id="ar_description" rows="3" class="mt-1 block w-full border rounded-md px-3 py-2">{{ old('ar_description') }}</textarea>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="form-group">
                ...
                <input type="number" name="display_order" id="display_order" value="{{ old('display_order', 0) }}" min="0" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            </div>
