...
<div class="main-content py-10">
  ...
  <form action="{{ route('admin.language-course-material-fees.store') }}" method="POST" class="space-y-4">
    @csrf
    ...
    <div>
      ...
      <select name="language_course_id" class="mt-1 block w-full border rounded-md px-3 py-2" required>
        ...
        @foreach($courses as $course)
          <option value="{{ $course->id }}" @selected(old('language_course_id') == $course->id)>
