...
@section('content')
    <div class="main-content py-10">
        <div class="flex justify-between py-10">
            <h2 class="text-2xl font-bold mb-4">Destinations List</h2>
            ...
            <ol class="flex items-center whitespace-nowrap min-w-0 pb-2">
                <li class="text-sm">
                    <a class="flex items-center text-primary hover:text-primary dark:text-primary"
                        ...
                        <svg class="flex-shrink-0 ltr:mr-3 rtl:ml-3 h-4 w-4 text-primary hover:text-primary dark:text-primary"
                ...
                <li class="text-sm">
                    <a class="flex items-center text-primary hover:text-primary dark:text-primary"
                        ...
                        <span class="flex items-center text-gray-500 dark:text-white/70">Destinations</span>
                    </a>
                </li>
            </ol>
        </div>
            ...
            <table class="min-w-full bg-white">
                <thead>
                    <tr class="border-b">
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 dark:text-white/70">Region</th>
                ...
                <tbody>
                    @forelse($destinations as $destination)
                        <tr class="border-b">
                            <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $destination->name }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $destination->slug }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $destination->country->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $destination->region }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-gray-500">No destinations found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
