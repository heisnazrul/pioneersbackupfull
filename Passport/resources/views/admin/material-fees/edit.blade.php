{{-- resources/views/admin/material-fees/edit.blade.php --}}
...
<div class="main-content py-10">
    ...
    <form action="{{ route('admin.material-fees.update', $materialFee) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="course_id" id="course_id"
                ...
                @foreach($courses as $course)
                    <option value="{{ $course->id }}" @selected(old('course_id', $materialFee->course_id) == $course->id)>
        ...
        <div>
            ...
            <input type="number" step="0.01" min="0" name="fee" id="fee" value="{{ old('fee', $materialFee->fee) }}"
                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm sm:text-sm" required>
        </div>
        ...
        <div>
            ...
            <select name="fee_type" id="fee_type"
                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm sm:text-sm" required>
                <option value="flat" @selected(old('fee_type', $materialFee->fee_type) == 'flat')>Flat</option>
                <option value="weekly" @selected(old('fee_type', $materialFee->fee_type) == 'weekly')>Weekly</option>
            </select>
        </div>
