{{-- resources/views/admin/registration-fees/edit.blade.php --}}
@extends('admin.layouts.layout')

@section('content')
<div class="main-content py-10">
    <div class="flex justify-between py-10">
        <h2 class="text-2xl font-bold mb-4">Edit Registration Fee</h2>
    ...
    <form action="{{ route('admin.registration-fees.update', $registrationFee) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
            ...
            <select name="branch_id" id="branch_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm sm:text-sm" required>
                @foreach($branches as $branch)
                    <option value="{{ $branch->id }}"
                        @selected(old('branch_id', $registrationFee->branch_id) == $branch->id)>
        ...
        <div>
            ...
            <input type="number" step="0.01" min="0" name="fee" id="fee"
                   value="{{ old('fee', $registrationFee->fee) }}"
