...
<div class="main-content py-10">
    <div class="flex justify-between py-10">
        ...
        <a href="{{ route('admin.pickups.create') }}" class="ti-btn rounded-full ti-btn-outline ti-btn-outline-primary">
            Add Pickup
        </a>
    </div>
  ...
  <div class="bg-white rounded-lg shadow-md p-4 mb-6">
    <form method="GET" action="{{ route('admin.pickups.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div>
      ...
      <div class="md:col-span-4 flex items-end gap-2">
        ...
        <a href="{{ route('admin.pickups.index') }}" class="ti-btn rounded-full border">Reset</a>
      </div>
    </form>
  </div>
        ...
        <table class="min-w-full bg-white">
            <thead>
                <tr class="border-b">
                    ...
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-500">Note</th>
            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="px-6 py-4 text-sm">{{ $pickup->city->name ?? '—' }}</td>
                    <td class="px-6 py-4 text-sm">{{ number_format($pickup->fee, 2) }}</td>
                    <td class="px-6 py-4 text-sm">{{ $pickup->note ?? '—' }}</td>
                    <td class="px-6 py-4 text-sm">
                        <a href="{{ route('admin.pickups.edit', $pickup) }}" class="text-blue-600 hover:text-blue-800">Edit</a>
                        <form action="{{ route('admin.pickups.destroy', $pickup) }}" method="POST" class="inline-block" onsubmit="return confirm('Delete this pickup?');">
                            @csrf
                            @method('DELETE')
