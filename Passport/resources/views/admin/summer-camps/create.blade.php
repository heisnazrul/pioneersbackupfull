...
<div class="main-content py-10">
  <div class="flex justify-between py-10">
    <h2 class="text-2xl font-bold mb-4">Create Summer Camp</h2>
  ...
  <form action="{{ route('admin.summer-camps.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <select name="status" class="mt-1 block w-full border rounded-md px-3 py-2" required>
          @foreach(['draft','published','suspended'] as $status)
            <option value="{{ $status }}" @selected(old('status','published') === $status)>{{ ucfirst($status) }}</option>
          @endforeach
        </select>
      </div>
      <div>
        ...
        <select name="branch_id" class="mt-1 block w-full border rounded-md px-3 py-2" required>
          ...
          @foreach($branches as $branch)
            <option value="{{ $branch->id }}" @selected(old('branch_id') == $branch->id)>{{ optional($branch->school)->name }} — {{ $branch->slug }}</option>
          @endforeach
        </select>
      </div>
      <div>
      ...
      <div>
        ...
        <input type="number" name="lessons_per_week" min="0" value="{{ old('lessons_per_week') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Age Range</label>
        <input type="text" name="age_range" value="{{ old('age_range') }}" class="mt-1 block w-full border rounded-md px-3 py-2" placeholder="e.g. 12-17">
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Start Date (text)</label>
        <input type="text" name="start_date" value="{{ old('start_date') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Payment Deadline</label>
        <input type="date" name="payment_deadline" value="{{ old('payment_deadline') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
      <div>
        ...
        <select name="fee_type" class="mt-1 block w-full border rounded-md px-3 py-2" required>
          @foreach(['flat','weekly'] as $type)
            <option value="{{ $type }}" @selected(old('fee_type','flat') === $type)>{{ ucfirst($type) }}</option>
          @endforeach
        </select>
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Fee Amount</label>
        <input type="number" name="fee_amount" step="0.01" min="0" value="{{ old('fee_amount') }}" class="mt-1 block w-full border rounded-md px-3 py-2" required>
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Registration Fee</label>
        <input type="number" name="registration_fee" step="0.01" min="0" value="{{ old('registration_fee') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
      <div>
        ...
        <p class="text-xs text-gray-500 mt-1">Upload up to 2MB.</p>
      </div>
      <div class="flex items-center space-x-2 pt-6">
        <input type="hidden" name="visible" value="0">
        <input type="checkbox" id="visible" name="visible" value="1" class="h-4 w-4" @checked(old('visible', true))>
    ...
    <div>
      ...
      <textarea name="description" rows="4" class="mt-1 block w-full border rounded-md px-3 py-2">{{ old('description') }}</textarea>
    </div>

    <div>
      <label class="block text-sm font-medium text-gray-700">Arabic Description</label>
      <textarea name="ar_description" rows="4" class="mt-1 block w-full border rounded-md px-3 py-2">{{ old('ar_description') }}</textarea>
    </div>

    <div class="flex space-x-2">
      <button type="submit" class="ti-btn rounded-full ti-btn-outline ti-btn-outline-success">Save Summer Camp</button>
