...
<div class="main-content py-10">
    <div class="flex justify-between py-10">
        <h2 class="text-2xl font-bold mb-4">Certification List</h2>
        <a href="{{ route('admin.certifications.create') }}" class="ti-btn rounded-full ti-btn-outline ti-btn-outline-primary">
        ...
        <ol class="flex items-center whitespace-nowrap min-w-0 pb-2">
            ...
            <li class="text-sm">
                <a class="flex items-center text-primary hover:text-primary dark:text-primary" href="javascript:void(0);">
                    <span class="flex items-center text-gray-500 dark:text-white/70">Certifications</span>
                </a>
            </li>
        </ol>
    </div>
    ...
    <div class="overflow-x-auto bg-white rounded-lg shadow-md">
        <table class="min-w-full bg-white">
            <thead>
                <tr class="border-b">
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 dark:text-white/70">title</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 dark:text-white/70">subtitle</th>
            ...
            <tbody>
                ...
                <tr class="border-b">
                    <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $certification->title }}</td>
                    <td class="px-6 py-4 text-sm">{{ $certification->ar_title }}</td>
                    <td class="px-6 py-4 text-sm">
                        @if($certification->certificate_image)
                           <img src="{{ asset('storage/'.ltrim($certification->certificate_image, '/')) }}" width="50" height="50" alt="Cetrification Picture" class="rounded border object-cover">
                        @else
                            No Image
                        @endif
                    </td>
                    <td class="px-6 py-4 text-sm">
                        <a href="{{ route('admin.certifications.edit', $certification) }}" class="text-blue-600 hover:text-blue-800">Edit</a>
                        <form action="{{ route('admin.certifications.destroy', $certification) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
